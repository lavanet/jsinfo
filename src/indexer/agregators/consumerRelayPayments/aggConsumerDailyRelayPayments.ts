...
export async function aggConsumerDailyRelayPayments(db: PostgresJsDatabase) {
    ...
    const aggResults = await db.select({
        ...
        qosSyncAvg: sql<number>`SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.qosSyncAvg} * ${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum}) / SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum})`,
        qosAvailabilityAvg: sql<number>`SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.qosAvailabilityAvg} * ${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum}) / SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum})`,
        qosLatencyAvg: sql<number>`SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.qosLatencyAvg} * ${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum}) / SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum})`,
        qosSyncExcAvg: sql<number>`SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.qosSyncExcAvg} * ${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum}) / SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum})`,
        qosAvailabilityExcAvg: sql<number>`SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.qosAvailabilityExcAvg} * ${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum}) / SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum})`,
        qosLatencyExcAvg: sql<number>`SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.qosLatencyExcAvg} * ${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum}) / SUM(${JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments.relaySum})`,
    }).from(JsinfoConsumerAgrSchema.aggConsumerHourlyRelayPayments)
        .where(
            and(
