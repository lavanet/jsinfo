...
export async function aggProviderDailyRelayPayments(db: PostgresJsDatabase) {
    ...
    const aggResults = await db.select({
        ...
        qosSyncExcAvg: sql<number>`SUM(${JsinfoProviderAgrSchema.aggHourlyRelayPayments.qosSyncExcAvg} * ${JsinfoProviderAgrSchema.aggHourlyRelayPayments.relaySum}) / SUM(${JsinfoProviderAgrSchema.aggHourlyRelayPayments.relaySum})`,
        qosAvailabilityExcAvg: sql<number>`SUM(${JsinfoProviderAgrSchema.aggHourlyRelayPayments.qosAvailabilityExcAvg} * ${JsinfoProviderAgrSchema.aggHourlyRelayPayments.relaySum}) / SUM(${JsinfoProviderAgrSchema.aggHourlyRelayPayments.relaySum})`,
        qosLatencyExcAvg: sql<number>`SUM(${JsinfoProviderAgrSchema.aggHourlyRelayPayments.qosLatencyExcAvg} * ${JsinfoProviderAgrSchema.aggHourlyRelayPayments.relaySum}) / SUM(${JsinfoProviderAgrSchema.aggHourlyRelayPayments.relaySum})`,
    }).from(JsinfoProviderAgrSchema.aggHourlyRelayPayments)
        .where(
            and(
