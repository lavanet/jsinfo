    ...
    switch (evt_name) {
        ...
        */
        ...
        case 'lava_relay_payment':
            ParseEventRelayPayment(evt, height, txHash, lavaBlock)
            break
        case 'lava_stake_new_provider':
            ParseEventStakeNewProvider(evt, height, txHash, lavaBlock)
            break
        // a successful provider stake entry modification
        case 'lava_stake_update_provider':
            ParseEventStakeUpdateProvider(evt, height, txHash, lavaBlock)
            break
        // a successful provider stake entry modification
        case 'lava_provider_unstake_commit':
            ParseEventProviderUnstakeCommit(evt, height, txHash, lavaBlock)
            break
        // freeze a provider in multiple chains
        case 'lava_freeze_provider':
            ParseEventFreezeProvider(evt, height, txHash, lavaBlock)
            break
        // unfreeze a provider in multiple chains 
        case 'lava_unfreeze_provider':
            ParseEventUnfreezeProvider(evt, height, txHash, lavaBlock)
            break
        // a successful provider report for unresponsiveness
        case 'lava_provider_reported':
            ParseEventProviderReported(evt, height, txHash, lavaBlock)
            break
        case 'lava_provider_jailed':
            ParseEventProviderJailed(evt, height, txHash, lavaBlock)
            break
        case 'lava_provider_temporary_jailed':
            ParseEventProviderTemporaryJailed(evt, height, txHash, lavaBlock)
            break
        // a successful report of latest block of a provider per chain
        case 'lava_provider_latest_block_report':
            ParseEventProviderLatestBlockReport(evt, height, txHash, lavaBlock)
            break
        ...
        */
        case 'lava_buy_subscription_event':
            ParseEventBuySubscription(evt, height, txHash, lavaBlock)
            break
        case 'lava_add_project_to_subscription_event':
            ParseEventAddProjectToSubscription(evt, height, txHash, lavaBlock)
            break
        case 'lava_del_project_to_subscription_event':
            ParseEventDelProjectToSubscription(evt, height, txHash, lavaBlock)
            break
        case 'lava_expire_subscription_event':
            ParseEventExpireSubscrption(evt, height, txHash, lavaBlock)
            break
        case 'lava_set_subscription_policy_event':
            ParseEventSetSubscriptionPolicyEvent(evt, height, txHash, lavaBlock)
            break
        ...
        */
        ...
        case 'lava_add_key_to_project_event':
            ParseEventAddKeyToProject(evt, height, txHash, lavaBlock)
            break

        // a successful deletion of a project key 
        case 'lava_del_key_from_project_event':
            ParseEventDelKeyFromProject(evt, height, txHash, lavaBlock)
            break
        ...
        case 'lava_conflict_vote_got_commit':
            ParseEventConflictVoteGotCommit(evt, height, txHash, lavaBlock)
            break
        ...
        case 'lava_response_conflict_detection':
            ParseEventResponseConflictDetection(evt, height, txHash, lavaBlock)
            break

        // redundant
        case 'lava_conflict_detection_received':
            ParseEventConflictDetectionReceived(evt, height, txHash, lavaBlock)
            break

        // provider revealed his vote
        case 'lava_conflict_vote_got_reveal':
            ParseEventConflictVoteGotReveal(evt, height, txHash, lavaBlock)
            break

        // conflict has transitioned to reveal state
        case 'lava_conflict_vote_reveal_started':
            ParseEventConflictVoteRevealStarted(evt, height, txHash, lavaBlock)
            break

        // conflict was succesfully resolved
        case 'lava_conflict_detection_vote_resolved':
            ParseEventConflictDetectionVoteResolved(evt, height, txHash, lavaBlock)
            break
        ...
        case 'lava_conflict_detection_vote_unresolved':
            ParseEventConflictDetectionVoteUnresolved(evt, height, txHash, lavaBlock)
            break
        ...
        case 'lava_delegate_to_provider':
            ...
            ParseEventDelegateToProvider(evt, height, txHash, lavaBlock)
            break

        // a successful provider delegation unbond
        case 'lava_unbond_from_provider':
            ParseEventUbondFromProvider(evt, height, txHash, lavaBlock)
            break

        // // a successful provider redelegation
        case 'lava_redelegate_between_providers':
            ParseEventRedelegateBetweenProviders(evt, height, txHash, lavaBlock)
            break

        // // a successful provider delegator reward claim
        case 'lava_delegator_claim_rewards':
            ParseEventDelegatorClaimRewards(evt, height, txHash, lavaBlock)
        ...
        case 'lava_validator_slash':
            ParseEventValidatorSlash(evt, height, txHash, lavaBlock)
            break

        case 'lava_freeze_from_unbond':
            ParseEventLavaFreezeFromUnbound(evt, height, txHash, lavaBlock)
            break

        case 'lava_unstake_from_unbond':
            ParseEventUnstakeFromUnbound(evt, height, txHash, lavaBlock)
            break
        ...
        default:
            ParseEventUnidentified(evt, height, txHash, lavaBlock)
