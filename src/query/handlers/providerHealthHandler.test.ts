...
describe('CompareHealthReportByStatusLatency', () => {
    test('should prioritize healthy over unhealthy and frozen', () => {
        ...
        expect(CompareHealthReportByStatusLatency(healthyReport, unhealthyReport)).toBeLessThan(0);
        expect(CompareHealthReportByStatusLatency(unhealthyReport, healthyReport)).toBeGreaterThan(0);
    });

    test('should prioritize unhealthy over frozen', () => {
        const unhealthyReport = createHealthReport({ status: 'unhealthy', latency: null, block: null, blocksaway: null, message: null });
        const frozenReport = createHealthReport({ status: 'frozen', latency: null, block: null, blocksaway: null, message: null });
        expect(CompareHealthReportByStatusLatency(unhealthyReport, frozenReport)).toBeLessThan(0);
        expect(CompareHealthReportByStatusLatency(frozenReport, unhealthyReport)).toBeGreaterThan(0);
    });
...
describe('applyHealthResponseGroupingAndTextFormatting', () => {
    it('correctly groups, sorts, and formats health reports', () => {
        ...
        const healthReports = [
            ...
            { timestamp: new Date('2024-01-01T10:10:00Z'), status: 'frozen', latency: 200, block: 100, blocksaway: 5, message: null, id: 1, provider: 'provider1', spec: 'specA', interface: 'interfaceA' },
            { timestamp: new Date('2024-01-01T10:12:00Z'), status: 'unhealthy', latency: 100, block: 105, blocksaway: 2, message: null, id: 2, provider: 'provider1', spec: 'specA', interface: 'interfaceA' },
            // A report in a different 15-minute interval
            { timestamp: new Date('2024-01-01T10:25:00Z'), status: 'healthy', latency: 300, block: 110, blocksaway: 1, message: null, id: 3, provider: 'provider1', spec: 'specA', interface: 'interfaceA' }
        ];
...
describe('GroupHealthReportBy15MinutesWithSort', () => {
    it('groups reports by 15-minute intervals and sorts them within groups', () => {
        const reports = [
            { id: 1, timestamp: new Date('2024-01-01T10:14:59Z'), status: 'frozen', latency: 200, block: 100, blocksaway: 1, message: 'Report 1', provider: 'providerA', spec: 'specA', interface: 'interfaceA' },
            { id: 2, timestamp: new Date('2024-01-01T10:15:01Z'), status: 'unhealthy', latency: 100, block: 100, blocksaway: 2, message: 'Report 2', provider: 'providerA', spec: 'specA', interface: 'interfaceA' },
            { id: 3, timestamp: new Date('2024-01-01T10:00:00Z'), status: 'healthy', latency: 300, block: 100, blocksaway: 3, message: 'Report 3', provider: 'providerA', spec: 'specA', interface: 'interfaceA' },
        ];
    ...
    it('groups reports by spec, interface, and 15-minute intervals', () => {
        const reports = [
            { id: 1, timestamp: new Date('2024-01-01T10:14:59Z'), status: 'frozen', latency: 200, block: 100, blocksaway: 1, message: 'Report 1', provider: 'providerA', spec: 'specA', interface: 'interfaceA' },
            { id: 2, timestamp: new Date('2024-01-01T10:15:01Z'), status: 'unhealthy', latency: 100, block: 100, blocksaway: 2, message: 'Report 2', provider: 'providerA', spec: 'specB', interface: 'interfaceB' },
            { id: 3, timestamp: new Date('2024-01-01T10:15:01Z'), status: 'healthy', latency: 300, block: 100, blocksaway: 3, message: 'Report 3', provider: 'providerA', spec: 'specB', interface: 'interfaceB' },
        ];

        const sortedAndGrouped = GroupHealthReportBy15MinutesWithSort(reports);

        // Verify the number of groups matches expectation
        expect(sortedAndGrouped.length).toBe(2);

        // Verify the groups have the correct spec and interface
        expect(sortedAndGrouped[0].spec).toBe('specA');
        expect(sortedAndGrouped[0].interface).toBe('interfaceA');
        expect(sortedAndGrouped[1].spec).toBe('specB');
        expect(sortedAndGrouped[1].interface).toBe('interfaceB');
    });
    ...
    test('should throw an error if HealthReport array contains items with different providers or null providers', () => {
        const report1 = createHealthReport({ provider: 'provider1', status: 'healthy', latency: null, block: null, blocksaway: null, message: null });
        const report2 = createHealthReport({ provider: 'provider2', status: 'healthy', latency: null, block: null, blocksaway: null, message: null });
        const report3 = createHealthReport({ provider: null, status: 'healthy', latency: null, block: null, blocksaway: null, message: null });

        expect(() => GroupHealthReportBy15MinutesWithSort([report1, report2])).toThrow('Not all items have the same provider');
        expect(() => GroupHealthReportBy15MinutesWithSort([report3, report1])).toThrow('Provider is null');
    });
